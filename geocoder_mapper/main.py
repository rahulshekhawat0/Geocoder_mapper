...
st.write(
    "Upload your **raw locations** (text) and your **DB locations** (with lat/lon). "
    "We geocode raw text (free via OpenStreetMap Nominatim) and map each to the nearest DB location. "
...
# =========================
INDIA_CC = "in"
RATE_DELAY_SEC = 1.05  # be nice to Nominatim
# List of States & UTs (normalized lowercase). Keep simple & robust to spelling.
INDIAN_STATES_UTS = {
    # States
...
def robust_read_csv(upload, expected_min_cols=1) -> pd.DataFrame:
    """
    Try multiple encodings and use csv.Sniffer to guess delimiter.
    """
    if upload is None:
    ...
    for enc in encodings:
        try:
            ...
            try:
                ...
                sep = None  # let pandas infer if possible
            ...
            last_err = e
    raise ValueError(
        f"Couldn't read CSV with tried encodings {encodings}. Last error: {last_err}"
    )
...
with st.sidebar:
    ...
    max_rows = st.number_input("Limit rows (for testing)", min_value=0, value=0, step=1,
                               help="0 = no limit")
    cutoff_km = st.number_input("Cutoff distance (km) to accept nearest DB match",
                                min_value=0.0, value=0.0, step=1.0,
                                help="0 = always accept nearest. If >0, rows farther than this will be 'No reliable match'.")
...
with left:
    raw_file = st.file_uploader("Upload RAW locations CSV", type=["csv"], help="Needs a column with free-text locations.")
with right:
    db_file = st.file_uploader("Upload DB locations CSV", type=["csv"], help="Preferred columns: db_location, lat, lon")

# Column selectors (populate after reading)
raw_col = None
db_name_col = None
db_lat_col = None
db_lon_col = None
...
if raw_file is not None:
    try:
        raw_df = robust_read_csv(raw_file, expected_min_cols=1)
...
if db_file is not None:
    try:
        db_df = robust_read_csv(db_file, expected_min_cols=1)
        st.success(f"DB loaded: {db_df.shape[0]} rows, {db_df.shape[1]} cols")
        # Try to auto-pick common column names
        ...
        db_lat_col = st.selectbox("Select DB latitude column (or 'None' if absent)", ["None"] + db_df.columns.tolist(), index=(db_df.columns.get_loc("lat")+1) if "lat" in db_df.columns else 0)
        db_lon_col = st.selectbox("Select DB longitude column (or 'None' if absent)", ["None"] + db_df.columns.tolist(), index=(db_df.columns.get_loc("lon")+1) if "lon" in db_df.columns else 0)
    except Exception as e:
...
def geocode_text(q: str, cc_bias: Optional[str]) -> Optional[Tuple[float, float, Dict]]:
    if not geocode or not q or not q.strip():
        return None
    q = normalize_txt(q)
    candidates = [q, f"{q}, India"] if (cc_bias or "").lower() == "in" else [q]
...
def ensure_db_latlon(df: pd.DataFrame, name_col: str, lat_col_opt: Optional[str], lon_col_opt: Optional[str],
    ...
    if has_lat and has_lon:
        ...
        if not allow_geocode_db:
            raise ValueError("DB lacks lat/lon and geocoding of DB is disabled. Enable it in sidebar.")
        if geocode is None:
            raise ValueError("Geocoder not ready. Provide a valid User-Agent.")

        # Geocode unique names
...
# =========================
# Mapping logic for each RAW row
# =========================
def map_one(raw_txt: str, db_tbl: pd.DataFrame, cutoff: float, cc_bias: Optional[str]) -> Dict:
    ...
    # Rule 1: If it's an Indian State/UT by text
...
if run_button:
    ...
    try:
        ...
        # Limit rows for testing if chosen
